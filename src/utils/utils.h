...
inline void InstallConstructor(v8::Isolate* isolate,
  ...
  v8::Local constructor_template{interface_template.As<v8::FunctionTemplate>()};
...
inline void InstallAttributes(v8::Isolate* isolate,
  ...
  v8::Local constructor_template{interface_template.As<v8::FunctionTemplate>()};
  v8::Local prototype_template{constructor_template->PrototypeTemplate()};
  v8::Local instance_template{constructor_template->InstanceTemplate()};
...
inline void InstallOperations(v8::Isolate* isolate,
  ...
  for (const auto& config : operations) {
    v8::Local act{v8::FunctionTemplate::New(
    ...
    if (config.dep == Dependence ::kPrototype) {
      prototype_template->Set(isolate, config.name, act,
                              config.propertyAttribute);
    } else if (config.dep == Dependence ::kInstance) {
      instance_template->Set(isolate, config.name, act,
                             config.propertyAttribute);
    } else if (config.dep == Dependence ::kConstruct) {
...
inline void ObjectAddFunction(v8::Local<v8::Object> target,
  ...
  v8::Local function_template{v8::FunctionTemplate::New(
...
inline v8::Local<v8::Value> GetExposedInterfaceObject(
  ...
  v8::Local function_template{wrapper_type_info->GetV8ClassTemplate(isolate)
                                  .As<v8::FunctionTemplate>()};
  v8::Local context{creation_context->GetCreationContextChecked()};
