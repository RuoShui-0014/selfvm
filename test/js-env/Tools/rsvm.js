...
Object.assign(rsvm, {
    ...
    blobMap: {},
    isWorker: false,
    // 所有context通过postMessage发送的信息都到这，用户可自己处理数据或分装为事件进行分发
    getMessageFromContext(msg) {
        debugger
        const ev = new Event("message");
        ev.data = msg;
        globalThis.onmessage(ev);
    },
    // 主线程通过Worker.postMessage发送信息到子线程中，
    // 都会回调子线程中的rsvm.getMessageFromMainThread方法，
    // 用户可自己处理数据或分装为事件进行分发
    getMessageFromMainThread(msg) {
        debugger
        const ev = new Event("message");
        ev.data = msg;
        globalThis.onmessage(ev);
    },
    // 子线程通过self.postMessage发送信息到主线程中，
    // 都会回调主线程中的rsvm.getMessageFromSubThread方法，
    // 用户可自己处理数据或分装为事件进行分发
    // 参数：target为worker对象，msg为数据
    getMessageFromSubThread(target, msg) {
        debugger
        JSON.parse(msg);
        const ev = new Event("message");
        ev.data = msg;
    ...
    promiseHook(promise, parent) {
        console.log("rsvm.promiseHook调用...");
    },
    // 需要返回error的stack值
    errorStackHook(error, sites) {
        if (error.message == "test error") {
            rsvm.callSites = sites;
            console.log(sites[0].toString(), sites[0].getScriptHash());
            sites[0].toString = ()=>{ return "----------" }
            console.log(error.stack, sites);
            error.message = "change error";
            return "dddddddd";
        }
    },
